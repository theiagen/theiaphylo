...
def main(args, output_file = 'phylocompare_results.txt'):
    ...
    tree_res = compare_trees(tree1, tree2, rooted = rooted, 
                             mc = args.matching_cluster, rf = args.robinson_foulds, lrm = args.lin_rajan_moret)

    # output the results
    output_results(output_file, tree_res)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description = 'Compare phylogenetic trees by distance metrics: ' \
    ...
    phy_args.add_argument('-o', '--outgroup', help = 'Comma-delimited list of outgroup tips for rooting trees')
    phy_args.add_argument('-m', '--midpoint', action = 'store_true', help = 'Root trees at midpoint')
    phy_args.add_argument('-mc', '--matching_cluster', action = 'store_true', help = 'Calculate matching cluster distance; overwrites default: ALL')
    phy_args.add_argument('-rf', '--robinson_foulds', action = 'store_true', help = 'Calculate Robinson-Foulds distance; overwrites default: ALL')
    phy_args.add_argument('-lrm', '--lin_rajan_moret', action = 'store_true', help = 'Calculate Lin-Rajan-Moret distance; overwrites default: ALL')

    run_args = parser.add_argument_group('Run Options')
    run_args.add_argument('-d', '--debug', action = 'store_true', help = 'Enable debug mode')
    ...
    if not args.matching_cluster and not args.robinson_foulds and not args.lin_rajan_moret:
